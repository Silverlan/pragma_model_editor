...
function util.ObjMeshObject:GetVertices() return self.m_verts end
function util.ObjMeshObject:GetUVCoordinates() return self.m_uvs end
function util.ObjMeshObject:GetNormals() return self.m_normals end
function util.ObjMeshObject:GetName() return self.m_name end
function util.ObjMeshObject:AddVertex(v) table.insert(self.m_verts,v) end
function util.ObjMeshObject:AddUVCoordinates(uv) table.insert(self.m_uvs,uv) end
function util.ObjMeshObject:AddNormal(n) table.insert(self.m_normals,n) end
function util.ObjMeshObject:AddFace(face) table.insert(self.m_faces,face) end
function util.ObjMeshObject:GetFaces() return self.m_faces end
function util.ObjMeshObject:CalculateUniqueVertices()
	...
	local function add_unique_vertex(pos,uv,n)
		...
		end]] -- Too expensive for large meshes; Optimization will have to be done by caller!
...
function ObjMesh:GetObjects() return self.m_objects end
...
ObjMesh.Load = function(fName)
	local f
	if(type(fName) == "string") then f = file.open(fName,"rb")
	else f = fName end
	if(f == nil) then return end
	local objects = {}
	local function get_object()
		local numObjs = #objects
		if(numObjs == 0) then table.insert(objects,util.ObjMeshObject("generic")) end
	...
	while(not f:Eof()) do
		...
		if(l[0] ~= "#") then
			...
			if(sp ~= nil) then
				...
				if(v == "o") then
					if(val:sub(-4) == ".obj") then val = val:sub(0,-5) end
