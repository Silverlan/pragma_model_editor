	...
	if numSkins > 1 then
		...
		for i = 1, numSkins do
			...
			pItem:AddCallback("OnSelectionChanged", function(pItem, selected)
				...
				if pContext.curSkin == nil or pSubMenu:GetSelectedItem() ~= nil then
					return
				end
		...
		for i, bg in ipairs(bodyGroups) do
			...
			for j = 1, #bg.meshGroups do
				...
				pItem:AddCallback("OnSelectionChanged", function(pItem, selected)
					...
					if pContext.curBg == nil or pContext.curBg[i] == nil or pSubMenu:GetSelectedItem() ~= nil then
						return
					end
...
function gui.WIModelView:SetClearColor(col)
	...
	if self.m_drawSceneInfo ~= nil then
...
local function draw_meshes(self, ent, meshes, shader, filter)
	...
	for _, t in ipairs(meshes) do
		...
		local subMesh = t[5]
		if
			self.m_bRenderFilterEnabled == false
			or self.m_renderFilter(ent, mdl, meshGroupId, meshGroup, mesh, subMesh, mat) == true
		then
			if filter == nil or filter(ent, mdl, meshGroupId, meshGroup, mesh, subMesh, mat) == true then
				if shader.RecordBindMaterial ~= nil then
					if shader:RecordBindMaterial(mat) == false then
						return false
					end
				end
				if shader.BindVertexAnimationOffset ~= nil then
					...
					if bBindVertexAnimBuffer == false then
						shader:BindVertexAnimationOffset(0)
					end
				end
				if shader:RecordDrawMesh(subMesh) == false then
					return false
				end
			end
		end
...
function gui.WIModelView:GetScene()
	return self.m_scene
end
function gui.WIModelView:GetRenderTarget()
	return self:GetScene():GetRenderer():GetRenderTarget()
end
function gui.WIModelView:GetPresentationTexture()
	return self:GetScene():GetRenderer():GetPresentationTexture()
end
function gui.WIModelView:GetRenderer()
	return self:GetScene():GetRenderer()
end
function gui.WIModelView:SetRenderFilterEnabled(b)
	self.m_bRenderFilterEnabled = b
end
...
function gui.WIModelView:CreateGroundEntity()
	if util.is_valid(self.m_entGround) then
		return self.m_entGround
	end
...
function gui.WIModelView:SetGroundVisible(visible)
	...
	if visible == false then
		if util.is_valid(self.m_entGround) == false then
			return
		end
...
function gui.WIModelView:GetLightSource()
	return self.m_entLight
end
function gui.WIModelView:CreateLightSource()
	...
	self.m_entLight = ent]]
	if util.is_valid(self.m_entLight) then
		return
	end
...
function gui.WIModelView:GetReflectionProbe()
	return self.m_entReflectionProbe
end
function gui.WIModelView:CreateReflectionProbe()
	if util.is_valid(self.m_entReflectionProbe) then
		return
	end
...
function gui.WIModelView:SetShowMeshes(b)
	self.m_bShowMeshes = b
end
function gui.WIModelView:ClearLightSources()
	for _, light in ipairs(self.m_lightSources) do
		if light:IsValid() == true then
			light:Remove()
		end
	end
	self.m_lightSources = {}
...
function gui.WIModelView:SetLightingEnabled(b)
	if b == self:IsLightingEnabled() then
		return
	end
...
function gui.WIModelView:IsLightingEnabled()
	return self.m_bLightingEnabled
end
...
function gui.WIModelView:RemoveActor(i)
	util.remove(self:GetEntity(i))
end
...
function gui.WIModelView:InitializeViewport(width, height)
	...
	if util.is_valid(ent) then
		vc:SetTarget(ent)
	end -- TODO: Take all entities into account
	...
	self.m_cbPrepareRendering = game.add_callback("PrepareRendering", function(renderer)
		if renderer ~= self.m_renderer then
			return
		end
...
function gui.WIModelView:DrawScene(immediate)
	...
	for i, ent in ipairs(self.m_actors) do
		if ent:IsValid() then
			if ent:HasComponent(ents.COMPONENT_PARTICLE_SYSTEM) then
				cycleChanged = true
			else
				...
				if animComponent ~= nil then
					...
					if
						nCycle ~= actorData.lastCycle
						and ((anim ~= nil and anim:GetFrameCount() > 1) or actorData.lastCycle == nil)
					then
						cycleChanged = true
					end
	...
	if self.m_alwaysRender ~= true and self.m_bRenderScheduled ~= true and cycleChanged == false then
		return
	end
	...
	local rt =
		self.m_scene:GetRenderer():GetEntity():GetComponent(ents.COMPONENT_RASTERIZATION_RENDERER):GetRenderTarget()
...
function gui.WIModelView:GetFrameIndex()
	return self.m_frameIndex
end
function gui.WIModelView:ScheduleEntityForRendering(shader, ent, filter)
	if ent == nil then
		for _, ent in ipairs(self.m_actors) do
			if ent:IsValid() then
				self:ScheduleEntityForRendering(shader, ent, filter)
			end
		end
		return
	end
		...
		for _, subMesh in ipairs(mesh:GetSubMeshes()) do
			...
			if mat ~= nil then
				if filter == nil or filter(ent, mdl, mesh, subMesh, mat) == true then
					if shader ~= nil then
						renderer:ScheduleMeshForRendering(game.SCENE_RENDER_PASS_WORLD, shader, mat, ent, subMesh)
					else
						renderer:ScheduleMeshForRendering(game.SCENE_RENDER_PASS_WORLD, mat, ent, subMesh)
					end
				end
			end
		end
...
function gui.WIModelView:GetCamera()
	return self.m_cam
end
...
function gui.WIModelView:GetParticleSystemName()
	return self.m_ptSystemName
end
...
function gui.WIModelView:ApplyModelFromEntity(entSrc, actorIdx)
	...
	if mdl == nil or util.is_valid(entDst) == false or self:SetModel(mdl, actorIdx) == false then
		return false
	end
	...
	if entSrc:HasComponent(ents.COMPONENT_EYE) then
		entDst:AddComponent(ents.COMPONENT_EYE)
	end
	return true
end
function gui.WIModelView:SetModel(mdl, actorIdx)
	if mdl ~= nil and type(mdl) == "string" then
		...
		if mdl ~= nil then
			return self:SetModel(mdl, actorIdx)
		end
		return false
	end
...
function gui.WIModelView:OnThink()
	...
	if self.m_bRotate ~= true and self.m_bMove ~= true then
		return
	end
...
function gui.WIModelView:MouseCallback(button, action, mods)
	...
	if button == input.MOUSE_BUTTON_LEFT then
		local handled, entActor, hitPos, startPos, hitData =
			ents.ClickComponent.inject_click_input(input.ACTION_ATTACK, action == input.STATE_PRESS)
	...
	if button == input.MOUSE_BUTTON_RIGHT then
		if action == input.STATE_PRESS then
			...
			if self.m_cursorTracker ~= nil then
				...
				if self.m_bMove and (self:GetCursorPos() - self.m_panningStartPos):LengthSqr() > 0.1 then
				...
				if mdl ~= nil then
					...
					gui.WIModelView.add_default_actor_context_options(pContext, ent, function()
						self:Render()
					end)
					if #anims > 1 then
						table.sort(anims)
						...
						for _, animName in ipairs(anims) do
							...
							if anim ~= nil and bit.band(anim:GetFlags(), game.Model.Animation.FLAG_GESTURE) == 0 then
								...
								pItem:AddCallback("OnSelectionChanged", function(pItem, selected)
									...
									if self.m_curAnim == nil or pSubMenu:GetSelectedItem() ~= nil then
										return
									end
						...
						for _, animName in ipairs(flexAnims) do
							...
							if anim ~= nil then
								...
								pItem:AddCallback("OnSelectionChanged", function(pItem, selected)
									...
									if pSubMenu:GetSelectedItem() ~= nil then
										return
									end
									local flexC = ent:GetComponent(ents.COMPONENT_FLEX)
									if flexC ~= nil then
					...
					pContext:Update()
					if pContext:GetItemCount() == 0 then
...
function gui.WIModelView:ScrollCallback(xoffset, yoffset)
	...
	if self.m_bCameraMovementEnabled == false then
		return
	end
	local vc = self:GetViewerCamera()
	if vc ~= nil then
...
function gui.WIModelView:GetCameraPosition()
	return self.m_cam:GetEntity():GetPos()
end
function gui.WIModelView:SetCameraPosition(pos)
	self.m_cam:GetEntity():SetPos(pos)
end
function gui.WIModelView:SetCameraRotation(rot)
	self.m_cam:GetEntity():SetRotation(rot)
end
...
function gui.WIModelView:OnUpdate()
	if util.is_valid(self.m_pBg) == false or util.is_valid(self.m_scene) == false then
		return
	end
...
function gui.WIModelView:SetAlwaysRender(alwaysRender)
...
function gui.WIModelView:PlayIdleAnimation()
	...
	if anim == -1 then
		anim = model:LookupAnimation("idle")
	end
	if anim == -1 then
	...
	if anim == -1 then
		anim = model:LookupAnimation("ragdoll")
	end
	if anim ~= -1 then
