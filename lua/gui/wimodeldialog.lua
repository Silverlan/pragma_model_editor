...
function gui.WIModelDialog:OnInitialize()
	...
	local mdlCatalog = gui.create("WIPFMModelCatalog",tabMdlExplorer,0,0,tabMdlExplorer:GetWidth(),tabMdlExplorer:GetHeight(),0,0,1,1)
	...
	explorer:AddCallback("OnIconAdded",function(explorer,icon)
		if(icon:IsDirectory() == false) then
			icon:AddCallback("OnSelectionChanged",function(icon,selected)
				if(icon:IsNativeAsset() == false) then icon:Reload(true) end
	...
	btCancel:AddCallback("OnPressed",function()
		if(self.m_fResultHandler ~= nil) then self.m_fResultHandler(gui.DIALOG_RESULT_CANCELLED) end
...
function gui.WIModelDialog:InitializeAssetImporter(pTabPanel,tabMdlExplorer)
	...
	local tAssetExplorer = gui.create("WIFileExplorer",tabAssetImporter,0,0,tabAssetImporter:GetWidth(),tabAssetImporter:GetHeight() -vbox:GetHeight(),0,0,1,1)
...
function gui.WIModelDialog:ImportModel(fileName)
	...
	if(ext == "pmx") then
		...
		f:Close()
		if(subMeshes == nil) then return end
		...
		for _,subMesh in ipairs(subMeshes) do
			...
			print("Added mesh with " .. subMesh:GetVertexCount() .. " vertices and " .. subMesh:GetTriangleCount() .. " triangles.")
		end
...
function gui.WIModelDialog:Close(result)
	gui.close_dialog()
	if(self.m_fResultHandler == nil) then return end
	if(result == gui.DIALOG_RESULT_OK) then
		local mdlName = self:GetModelName()
		if(mdlName == nil) then self.m_fResultHandler(gui.DIALOG_RESULT_NO_SELECTION)
		else self.m_fResultHandler(gui.DIALOG_RESULT_OK,mdlName) end
		return
	end
	self.m_fResultHandler(result)
end
function gui.WIModelDialog:SetResultHandler(fResultHandler) self.m_fResultHandler = fResultHandler end
function gui.WIModelDialog:GetModelName() return self.m_modelName end
