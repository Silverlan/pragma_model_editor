...
function gui.WIModelEditorHitboxes:SetModel(mdl)
	...
	if(util.is_valid(pHitboxes) == false) then return end
	pHitboxes:Clear()
	if(util.is_valid(mdl) == false or self.m_shader == nil or util.is_valid(ent) == false or util.is_valid(pEditor) == false) then return end
	...
	for hitboxId,boneId in ipairs(hitboxBones) do
		...
		if(hitGroup == game.HITGROUP_HEAD) then col = Color.Red; hitGroupName = locale.get_text("head")
		elseif(hitGroup == game.HITGROUP_CHEST) then col = Color.Lime; hitGroupName = locale.get_text("chest")
		elseif(hitGroup == game.HITGROUP_STOMACH) then col = Color.Blue; hitGroupName = locale.get_text("stomach")
		elseif(hitGroup == game.HITGROUP_LEFT_ARM) then col = Color.Yellow; hitGroupName = locale.get_text("left_arm")
		elseif(hitGroup == game.HITGROUP_RIGHT_ARM) then col = Color.Cyan; hitGroupName = locale.get_text("right_arm")
		elseif(hitGroup == game.HITGROUP_LEFT_LEG) then col = Color.Magenta; hitGroupName = locale.get_text("left_leg")
		elseif(hitGroup == game.HITGROUP_RIGHT_LEG) then col = Color.OrangeRed; hitGroupName = locale.get_text("right_leg")
		elseif(hitGroup == game.HITGROUP_GEAR) then col = Color.SpringGreen; hitGroupName = locale.get_text("gear") end
		col = col:ToVector4()
  ...
  bufCreateInfo.size = dsIndices:GetSize()
	...
	if(bufBoneIndices == nil) then return end
...
function gui.WIModelEditorHitboxes:Render(drawCmd,cam)
	local ent = self:GetEntity()
	if(self:GetShowItems() == false or self.m_shader == nil or util.is_valid(ent) == false) then return end
	local offset
	local offsetOutline
	local vertCount
	local vertCountOutline
	if(self:IsSelected() == true and self.m_selectedHitbox ~= nil and self.m_tHitboxOffset[1][self.m_selectedHitbox] ~= nil) then
	...
	if(util.is_valid(self.m_bufHitboxVerts) == true) then self.m_shader:Draw(drawCmd,ent,self.m_bufHitboxVerts,self.m_bufHitboxBoneIndices,cam:GetProjectionMatrix() *cam:GetViewMatrix(),vertCountOutline,offsetOutline) end
	if(util.is_valid(self.m_bufFlatVerts) == true) then self.m_shaderFlat:Draw(drawCmd,ent,self.m_bufFlatVerts,self.m_bufFlatBoneIndices,cam:GetProjectionMatrix() *cam:GetViewMatrix(),vertCount,offset) end
end
