...
function gui.WIModelEditorInfo:AddOption(name,identifier)
	...
	pCbOption:AddCallback("OnChange",function(pCb,b)
		...
		if(util.is_valid(pEditor)) then pEditor:ScheduleEntityForRendering() end
...
function gui.WIModelEditorInfo:OnInitialize()
	...
	bt:AddCallback("OnPressed",function(pButton)
		...
		local ang = EulerAngles((rot[1] ~= nil) and tonumber(rot[1]) or 0.0,(rot[2] ~= nil) and tonumber(rot[2]) or 0.0,(rot[3] ~= nil) and tonumber(rot[3]) or 0.0)
...
function gui.WIModelEditorInfo:SetModel(mdl)
	if(util.is_valid(mdl) == false) then
		for _,pInfo in ipairs(self.m_tInfos) do
			if(util.is_valid(pInfo) == true) then pInfo:Remove() end
		end
		self.m_tInfos = {}
		return
	end
	...
	local i = 1
	self:SetInfoText(i,locale.get_text("mde_info_model_name") .. ": " .. mdl:GetName()); i = i +1
	self:SetInfoText(i,locale.get_text("mass") .. ": " .. mdl:GetMass()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_vertex_count") .. ": " .. mdl:GetVertexCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_triangle_count") .. ": " .. mdl:GetTriangleCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_bone_count") .. ": " .. mdl:GetBoneCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_hitbox_count") .. ": " .. mdl:GetHitboxCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_animation_count") .. ": " .. mdl:GetAnimationCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_attachment_count") .. ": " .. mdl:GetAttachmentCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_blend_controller_count") .. ": " .. mdl:GetBlendControllerCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_material_count") .. ": " .. mdl:GetMaterialCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_texture_group_count") .. ": " .. mdl:GetTextureGroupCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_mesh_group_count") .. ": " .. mdl:GetMeshGroupCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_mesh_count") .. ": " .. mdl:GetMeshCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_base_mesh_count") .. ": " .. #mdl:GetBaseMeshGroupIds()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_collision_mesh_count") .. ": " .. mdl:GetCollisionMeshCount()); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_render_bounds") .. ": (" .. tostring(minRender) .. ") (" .. tostring(maxRender) .. ")"); i = i +1
	self:SetInfoText(i,locale.get_text("mde_info_collision_bounds") .. ": (" .. tostring(minCol) .. ") (" .. tostring(maxCol) .. ")"); i = i +1
...
function gui.WIModelEditorInfo:OnSizeChanged(w,h)
	if(util.is_valid(self.m_optionsBox)) then self.m_optionsBox:SetX(w -self.m_optionsBox:GetWidth() -20) end
end
