...
function WIModelEditorWireframeMesh:__init(verts,colors)
	self.m_bValid = false
	if(#verts ~= #colors) then return end
	local shaderWireframe = shader.get("mde_wireframe_simple")
	if(shaderWireframe == nil) then return end
  ...
  bufCreateInfo.memoryFeatures = prosper.MEMORY_FEATURE_GPU_BULK_BIT
	...
	if(bufVert == nil) then return end
...
function WIModelEditorWireframeMesh:IsValid() return self.m_bValid end
function WIModelEditorWireframeMesh:Draw(rt,m)
	if(self:IsValid() == false or self.m_shader == nil) then return end
