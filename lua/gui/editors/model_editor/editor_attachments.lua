...
function gui.WIModelEditorAttachments:OnInitialize()
	...
	pButtonAdd:AddCallback("OnPressed",function(pButtonAdd)
		...
		mdl:AddAttachment(teName:GetText(),teBone:GetText(),vector.create_from_string(teOffset:GetText()),angle.create_from_string(teAngles:GetText()))
		self:SetModel(mdl)
	end)
	...
	pEdit:AddCallback("OnPressed",function(pEdit)
		...
		if(pRow ~= nil) then pRow:Select() end
	end)
	self.m_pEdit = pEdit
end
function gui.WIModelEditorAttachments:Render(drawCmd,cam)
	local pEditor = self:GetEditor()
	if(self.m_attachment == nil or util.is_valid(pEditor) == false) then return end
...
function gui.WIModelEditorAttachments:SetModel(mdl)
	...
	if(util.is_valid(pAttachments) == false) then return end
	pAttachments:Clear()
...
function gui.WIModelEditorAttachments:OnSizeChanged(w,h)
	...
	if(util.is_valid(self.m_pAttachments) == false) then return end
