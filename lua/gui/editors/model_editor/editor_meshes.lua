...
function gui.WIModelEditorMeshes:PrepareRendering(pModelView)
	...
	pModelView:ScheduleEntityForRendering(nil,nil,function(ent,mdl,mesh,subMesh,mat)
		if(self.m_selectedMesh ~= nil) then return mesh == self.m_selectedMesh end
		if(self.m_selectedSubMesh ~= nil) then return subMesh == self.m_selectedSubMesh end
		return false
	end)
end
function gui.WIModelEditorMeshes:Render(drawCmd,cam)
	...
	pModelView:RenderEntity(nil,function(ent,mdl,meshGroupId,meshGroup,mesh,subMesh,mat)
		...
		if(self.m_selectedMesh ~= nil and mesh ~= self.m_selectedMesh) then return false end
		if(self.m_selectedSubMesh ~= nil and subMesh ~= self.m_selectedSubMesh) then return false end
		return true
	end,drawCmd)
end
function gui.WIModelEditorMeshes:SetSelected(b)
	...
	if(b == true) then pModelView:SetShowMeshes(false)
	else pEditor:UpdateShowMeshes() end
end
function gui.WIModelEditorMeshes:SetModel(mdl)
	local pMeshes = self.m_pMeshes
	if(util.is_valid(pMeshes) == false) then return end
	pMeshes:Clear()

	if(util.is_valid(mdl) == false) then return end
	local baseMeshIds = {}
	for _,id in ipairs(mdl:GetBaseMeshGroupIds()) do baseMeshIds[id] = true end
	for groupId,meshGroup in ipairs(mdl:GetMeshGroups()) do
		local name = meshGroup:GetName()
		if(baseMeshIds[groupId -1] == true) then name = name .. " (Base mesh)" end
		...
		for meshId,mesh in ipairs(meshes) do
			local pElMesh = pEl:AddItem("Mesh #" .. meshId .. " (" .. mesh:GetVertexCount() .. " vertices) (" .. mesh:GetTriangleCount() .. " triangles)")
			...
			for subMeshId,subMesh in ipairs(mesh:GetSubMeshes()) do
				local pElSubMesh = pElMesh:AddItem("SubMesh #" .. subMeshId .. " (" .. subMesh:GetVertexCount() .. " vertices) (" .. subMesh:GetTriangleCount() .. " triangles)")
						...
						for groupId,meshGroup in ipairs(mdl:GetMeshGroups()) do
							for meshId,mesh in ipairs(meshGroup:GetMeshes()) do
								for subMeshId,subMeshOther in ipairs(mesh:GetSubMeshes()) do
									if(util.is_same_object(subMesh,subMeshOther) == false and skinTexId == subMeshOther:GetSkinTextureIndex()) then
										isDistinct = false
										break
									end
								end
								if(isDistinct == false) then break end
							end
							if(isDistinct == false) then break end
						end
						if(isDistinct == true) then return util.EVENT_REPLY_UNHANDLED end
