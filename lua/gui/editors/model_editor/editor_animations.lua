...
function gui.WIModelEditorAnimations:InitializeEvents(pTabEvents)
	...
	pFrameSlider:AddCallback("OnChange",function(pSlider,progress,value)
		if(self:IsValid() == false or self.m_pFrameSlider:IsValid() == false) then return end
	...
	pButtonApplyChanges:AddCallback("OnPressed",function(pButton)
		...
		if(pRow == nil) then return end
		if(util.is_valid({self.m_pType,self.m_pArguments,anim}) == false or self.m_selectedEventIdx == nil) then return end
		local evType = tonumber(self.m_pType:GetText()) or tonumber(self.m_pType:GetValue())
		if(evType == nil) then return end
	...
	pButtonAddEvent:AddCallback("OnPressed",function(pButton)
		if(self:IsValid() == false or pFrameSlider:IsValid() == false or pTypes:IsValid() == false) then return end
		local anim = self:GetSelectedAnimation()
		if(anim == nil) then return end
		local evType = tonumber(pTypes:GetValue()) or tonumber(pTypes:GetText())
		if(evType == nil) then return end
		local args = {}
		if(util.is_valid(self.m_pArguments) == true) then args = string.split(self.m_pArguments:GetText(),",") end
	...
	pButtonAddEvent:SizeToContents()
	pButtonAddEvent:AddCallback("OnPressed",function(pButton)
		if(self:IsValid() == false or self.m_selectedEventIdx == nil) then return end
		local anim = self:GetSelectedAnimation()
		if(anim == nil) then return end
		local pRow = self.m_pEvents:GetFirstSelectedRow()
		if(pRow == nil) then return end
...
function gui.WIModelEditorAnimations:InitializeAnimation(pTabAnimation)
	...
	local function set_anim_flag(flag,b)
		...
		if(b == true) then anim:AddFlags(flag)
		else anim:RemoveFlags(flag) end
	end
	...
	self.m_pCbAutoplay = self:AddFlag(locale.get_text("mde_anim_flag_autoplay"),function(pCb,b)
		...
		if(b == true and util.is_valid(self.m_pCbGesture) == true) then self.m_pCbGesture:SetChecked(b) end
...
function gui.WIModelEditorAnimations:OnInitialize()
	...
	pFrameSlider:AddCallback("OnChange",function(pSlider,progress,value)
		if(self:IsValid() == false or self.m_bUpdateCycle == false) then return end
		local ent = self:GetEntity()
		if(util.is_valid(ent) == true) then
			if(util.is_valid(self.m_pSpeedSlider) == true) then self.m_pSpeedSlider:SetValue(0) end
			local animComponent = ent:GetComponent(ents.COMPONENT_ANIMATED)
			if(animComponent ~= nil) then animComponent:SetCycle(pSlider:GetProgress()) end
		end
	end)
...
function gui.WIModelEditorAnimations:OnThink()
	local ent = self:GetEntity()
	if(util.is_valid(self.m_pFrameSlider) == false or util.is_valid(ent) == false) then return end
	local mdlComponent = ent:GetComponent(ents.COMPONENT_ANIMATED)
	local anim = self:GetSelectedAnimation()
	if(mdlComponent == nil or anim == nil) then return end
...
function gui.WIModelEditorAnimations:GetSelectedAnimation() return self.m_selectedAnimation end
function gui.WIModelEditorAnimations:SetSelectedAnimation(anim)
	...
	if(util.is_valid(self.m_pCbLoop) == true) then self.m_pCbLoop:SetChecked(bit.band(flags,Animation.FLAG_LOOP) ~= 0) end
	if(util.is_valid(self.m_pCbNoRepeat) == true) then self.m_pCbNoRepeat:SetChecked(bit.band(flags,Animation.FLAG_NOREPEAT) ~= 0) end
	if(util.is_valid(self.m_pCbMoveX) == true) then self.m_pCbMoveX:SetChecked(bit.band(flags,Animation.FLAG_MOVEX) ~= 0) end
	if(util.is_valid(self.m_pCbMoveY) == true) then self.m_pCbMoveY:SetChecked(bit.band(flags,Animation.FLAG_MOVEZ) ~= 0) end
	if(util.is_valid(self.m_pCbAutoplay) == true) then self.m_pCbAutoplay:SetChecked(bit.band(flags,Animation.FLAG_AUTOPLAY) ~= 0) end
	if(util.is_valid(self.m_pCbGesture) == true) then self.m_pCbGesture:SetChecked(bit.band(flags,Animation.FLAG_GESTURE) ~= 0) end
	if(util.is_valid(self.m_pFrameSlider) == true) then self.m_pFrameSlider:SetRange(0,numFrames) end
	if(util.is_valid(self.m_pEventFrameSlider) == true) then self.m_pEventFrameSlider:SetRange(0,numFrames) end
	if(util.is_valid(self.m_pFPS) == true) then self.m_pFPS:SetText(tostring(fps)) end
	if(util.is_valid(self.m_pActivity) == true) then self.m_pActivity:SelectOption(tostring(act)) end
	if(util.is_valid(self.m_pActivityWeight) == true) then self.m_pActivityWeight:SetText(tostring(actWeight)) end
...
function gui.WIModelEditorAnimations:ReloadBlendControllers()
	...
	if(util.is_valid(self.m_pBlendControllers) == false) then return end
	local pBlendControllers = self.m_pBlendControllers
	pBlendControllers:Clear()
	local pEditor = self:GetEditor()
	if(util.is_valid(pEditor) == false) then return end
...
function gui.WIModelEditorAnimations:ReloadEvents()
	self.m_selectedEventIdx = nil
	if(util.is_valid(self.m_pEvents) == false) then return end
...
function gui.WIModelEditorAnimations:AddFlag(name,fc,tooltip)
	if(util.is_valid(self.m_pAnimGridPanel) == false) then return end
	...
	if(tooltip ~= nil) then pLb:SetTooltip(tooltip) end
...
function gui.WIModelEditorAnimations:SetModel(mdl)
	...
	if(util.is_valid(pAnimations) == false) then return end
	pAnimations:Clear()
	...
	for _,anim in ipairs(tAnims) do
		...
		pRow:AddCallback("OnSelected",function(pRow)
			...
			if(util.is_valid(pCell) == false) then return end
			local pText = pCell:GetFirstChild("WIText")
			if(util.is_valid(pText) == false) then return end
...
function gui.WIModelEditorAnimations:ShowControls(b)
	if(util.is_valid(self.m_pAnimGridPanel) == true) then self.m_pAnimGridPanel:SetVisible(b) end
	if(util.is_valid(self.m_pEventContainer) == true) then self.m_pEventContainer:SetVisible(b) end
	if(util.is_valid(self.m_pBlendControllerContainer) == true) then self.m_pBlendControllerContainer:SetVisible(b) end
	if(util.is_valid(self.m_pFrameSlider) == true) then self.m_pFrameSlider:SetVisible(b) end
	if(util.is_valid(self.m_pLabelFrame) == true) then self.m_pLabelFrame:SetVisible(b) end
	if(util.is_valid(self.m_pSpeedSlider) == true) then self.m_pSpeedSlider:SetVisible(b) end
	if(util.is_valid(self.m_pLabelSpeed) == true) then self.m_pLabelSpeed:SetVisible(b) end
end
function gui.WIModelEditorAnimations:OnSizeChanged(w,h)
	gui.Base.OnSizeChanged(self,w,h)

	if(util.is_valid(self.m_pAnimations) == false) then return end
	...
	if(util.is_valid(self.m_pTabPanel) == false) then return end
