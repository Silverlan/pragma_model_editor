...
function gui.WIModelEditorFlexes:UpdateOptions()
	...
	if(util.is_valid(self.m_pFlexControllers) == true) then self.m_pFlexControllers:SetHeight(height *0.4) end
	if(util.is_valid(self.m_pFlexes) == true) then
		self.m_pFlexes:SetHeight(height *0.4)
		if(util.is_valid(self.m_pFlexControllers) == true) then self.m_pFlexes:SetY(self.m_pFlexControllers:GetBottom() +10) end
	end
	local yStart = MARGIN_OFFSET
	if(util.is_valid(self.m_pFlexControllers)) then yStart = yStart +self.m_pFlexControllers:GetBottom() end
	local y = yStart
...
function gui.WIModelEditorFlexes:SetModel(mdl)
	...
	if(util.is_valid(self.m_pFlexControllers)) then self.m_pFlexControllers:Clear() end
	if(util.is_valid(self.m_pFlexes)) then self.m_pFlexes:Clear() end
	self.m_tOptions = {}
	self.m_tFlexSliders = {}
