...
local fImport = function(pEditor,mdl,mdlName,fOpen,importType,tFiles)
	...
	if(p ~= nil) then mdlName = mdlName:sub(1,p -1) end
	...
	for _,f in ipairs(tFiles) do
		...
		local fName = f
		local p = fName:find('%.')
		if(p ~= nil) then fName = fName:sub(1,p -1) end
		if(fName == mdlName) then
			if(ext ~= nil) then
				if(ext == "ani") then fNames["ani"] = f
				elseif(ext == "mdl") then fNames["mdl"] = f
				elseif(ext == "phy") then fNames["phy"] = f
				elseif(ext == "vvd") then fNames["vvd"] = f
				elseif(ext == "vtx") then
					local p = f:find('%.')
	...
	if(fNames["mdl"] == nil) then return false end
	local files = {}
	for ext,fName in pairs(fNames) do
		files[ext] = game.open_dropped_file(fName,true)
	end
	if(files["mdl"] == nil) then return false end
...
for _,ext in ipairs({"ani","vtx","mdl","phy","vvd"}) do
	gui.WIModelEditor.register_importer(ext,bit.bor(gui.WIModelEditor.IMPORT_TYPE_MESH,gui.WIModelEditor.IMPORT_TYPE_ANIMATION,gui.WIModelEditor.IMPORT_TYPE_PHYSICS),fImport)
end
