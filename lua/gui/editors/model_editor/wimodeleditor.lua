...
util.register_class("gui.WIModelEditor",gui.WIBaseEditor)
...
gui.WIModelEditor.IMPORT_TYPE_MESH = 1
gui.WIModelEditor.IMPORT_TYPE_PHYSICS = 2
gui.WIModelEditor.IMPORT_TYPE_ANIMATION = 4
gui.WIModelEditor.importers = {}

gui.WIModelEditor.register_importer = function(ext,importTypes,fImport)
	gui.WIModelEditor.importers[ext:lower()] = {importTypes,fImport}
end
...
function gui.WIModelEditor:__init()
	gui.WIBaseEditor.__init(self)

	include("/shaders/mde")
end
function gui.WIModelEditor:OnRemove()
	gui.WIBaseEditor.OnRemove(self)

	if(util.is_valid(self.m_cbDropped) == true) then self.m_cbDropped:Remove() end
	if(util.is_valid(self.m_entLightSource)) then self.m_entLightSource:Remove() end
end
function gui.WIModelEditor:AddPanel(pTab,p)
	table.insert(self.m_tShowPanel,false)
	table.insert(self.m_tPanels,p)
	table.insert(self.m_tTabs,pTab)
	return #self.m_tPanels
end
function gui.WIModelEditor:GetPanel(i)
	return self.m_tPanels[i]
end
function gui.WIModelEditor:LogMessage(msg)
	-- TODO
end
function gui.WIModelEditor:UpdateShowMeshes()
	if(util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:SetShowMeshes(self.m_bShowMeshes)
end
function gui.WIModelEditor:OnInitialize()
	...
	self.m_menuBar:AddItem(locale.get_text("file"),function(pContext)
		pContext:AddItem(locale.get_text("open") .. "...",function(pItem)
			...
			local dialoge = gui.create_file_open_dialog(function(pDialoge,file)
				self:Clear()
				self:SetModel(file:sub(8))
			end)
			dialoge:SetRootPath("models")
			dialoge:SetExtensions({"wmd"})
			dialoge:Update()
		end)
		pContext:AddItem(locale.get_text("import") .. "...",function(pItem)
			-- TODO: Set import type and retrieve all appropriate importers from self.importers
			if(util.is_valid(self) == false) then return end
			local dialoge = gui.create_file_open_dialog(function(pDialoge,file)
				self:Clear()
				self:SetModel(file:sub(8)) -- Strip "models/"-prefix
			end)
			dialoge:SetRootPath("models")
			dialoge:SetExtensions({"wmd","smd"})
			dialoge:Update()
		end)
	...
	self.m_menuBar:AddItem(locale.get_text("view"),function(pContext)
		pContext:AddItem(locale.get_text("mde_reset_camera"),function(pItem)
			if(util.is_valid(self) == false or util.is_valid(self.m_pModelView) == false) then return end
			self.m_pModelView:ResetCamera()
		end)
		pContext:Update()
	end)

	self.m_container = gui.create("WIVBox",self,0,self.m_menuBar:GetBottom(),self:GetWidth(),self:GetHeight() -self.m_menuBar:GetBottom(),0,0,1,1)
	...
	pModelView:AddCallback("PrepareRendering",function(pModelView)
		...
		for _,p in ipairs(self.m_tPanels) do
			if(util.is_valid(p) == true) then p:PrepareRendering(pModelView) end
		end
	end)
	...
	pInfo:AddCallback("OnOptionChanged",function(pInfo,name,pCb,bActivated)
		...
		elseif(name == "show_ground") then
			...
			if(util.is_valid(self.m_pModelView)) then self.m_pModelView:SetGroundVisible(bActivated) end
	...
	self.m_cbDropped = game.add_callback("OnFilesDropped",function(tFiles)
		if(self:IsValid() == false or util.is_valid(self.m_model) == false) then return end
		local update = 0
		local textureFiles = {}
		for i,fName in ipairs(tFiles) do
			local ext = file.get_file_extension(fName)
			if(ext ~= nil and (ext == ".ktx" or ext == ".dds" or ext == ".png" or ext == ".tga" or ext == ".vtf")) then table.insert(textureFiles,{fName,fName:sub(1,-(#ext +1))}) end
		end
		...
		local function save_textures(mdlName,textures)
			...
			for _,tex in ipairs(textures) do
				...
				if(ext ~= nil) then tex = tex:sub(1,-#ext -2) end
						...
						if(fIn ~= nil) then
							...
							if(fOut ~= nil) then
								if(bFirst == true) then
									...
									self:LogMessage("All model textures have been copied to '" .. relTexPath .. "' and appropriate material files have been generated!")
									bFirst = false
								end
								bValidTexture = true
								...
								if(fMat ~= nil) then
									fMat:WriteString("\"textured\"\n{\n\t$texture diffusemap \"" .. relTexPath .. "/" .. f[2] .. "\"\n}\n")
									fMat:Close()
								end
							end
							fIn:Close()
				...
				self:LogMessage("WARNING: Texture '" .. tex .. "' required for model '" .. mdlName .. "' not found in input files!")
			end
		end
		local nFiles = #tFiles
		while(nFiles > 0) do
			...
			if(ext ~= nil) then
				...
				local type = (self.m_pSelectedTab == self.m_pTabPhysics) and self.IMPORT_TYPE_PHYSICS or self.IMPORT_TYPE_MESH
				if(importer ~= nil and bit.band(importer[1],type) ~= 0) then
					...
					if(r == true) then
						update = 3
						if(textures ~= nil) then save_textures(mdlName,textures) end
					end
				end
			end
			nFiles = #tFiles
		...
		_x = self.m_model
		if(bit.band(update,2) ~= 0) then self.m_model:Update() end
		if(bReloadMaterials == true) then self.m_model:LoadMaterials(true) end
		if(bit.band(update,1) ~= 0) then self:SetModel(self.m_model) end
	end)
...
local boxVerts = {
	startPos,Vector(startPos.x,startPos.y,endPos.z),
	startPos,Vector(endPos.x,startPos.y,startPos.z),
	startPos,Vector(startPos.x,endPos.y,startPos.z),
	Vector(startPos.x,startPos.y,endPos.z),Vector(endPos.x,startPos.y,endPos.z),
	Vector(startPos.x,startPos.y,endPos.z),Vector(startPos.x,endPos.y,endPos.z),
	Vector(endPos.x,startPos.y,endPos.z),Vector(endPos.x,startPos.y,startPos.z),
	Vector(endPos.x,startPos.y,endPos.z),endPos,
	Vector(endPos.x,startPos.y,startPos.z),Vector(endPos.x,endPos.y,startPos.z),
	Vector(startPos.x,endPos.y,startPos.z),Vector(startPos.x,endPos.y,endPos.z),
	Vector(startPos.x,endPos.y,startPos.z),Vector(endPos.x,endPos.y,startPos.z),
	endPos,Vector(endPos.x,endPos.y,startPos.z),
	endPos,Vector(startPos.x,endPos.y,endPos.z)
}
function gui.WIModelEditor:GetBoxVertices()
	return boxVerts
end
function gui.WIModelEditor:PrepareRendering(pModelView)
	if(self.m_bShowWireframe) then pModelView:ScheduleEntityForRendering(self.m_shaderWireframe) end
	if(self.m_bShowVertices) then pModelView:ScheduleEntityForRendering(self.m_shaderPoints) end
end
function gui.WIModelEditor:ScheduleEntityForRendering()
	if(util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:Render()
end
function gui.WIModelEditor:DrawAxis(m)
	if(util.is_valid(self.m_axisWireframeMesh) == false or util.is_valid(self.m_pModelView) == false) then return end
...
function gui.WIModelEditor:DrawBox(m)
	if(util.is_valid(self.m_boxWireframeMesh) == false or util.is_valid(self.m_pModelView) == false) then return end
	local rt = self.m_pModelView:GetRenderTarget()
	local cam = self.m_pModelView:GetCamera()
	m = cam:GetProjectionMatrix() *cam:GetViewMatrix() *m
	self.m_boxWireframeMesh:Draw(rt,m)
end
function gui.WIModelEditor:DrawMesh(mesh,m)
	if(util.is_valid(self.m_pModelView) == false) then return end
	local rt = self.m_pModelView:GetRenderTarget()
	local cam = self.m_pModelView:GetCamera()
	m = cam:GetProjectionMatrix() *cam:GetViewMatrix() *m
...
function gui.WIModelEditor:GetModel()
	if(util.is_valid(self.m_pModelView) == false) then return end
	return self.m_pModelView:GetModel()
end
function gui.WIModelEditor:GetModelName() return self.m_modelName end
function gui.WIModelEditor:GetEntity()
	if(util.is_valid(self.m_pModelView) == false) then return end
	return self.m_pModelView:GetEntity()
end
function gui.WIModelEditor:SetLOD(lod)
	self.m_lodMeshes = {}
	local mdl = self:GetModel()
	if(util.is_valid(mdl) == false) then return end
	self.m_lodMeshes = mdl:GetBodyGroupMeshes(lod)
end
function gui.WIModelEditor:FilterMeshes(ent,mdl,meshGroupId,meshGroup,mesh,subMesh,mat)
	for _,meshLod in ipairs(self.m_lodMeshes) do
		if(meshLod == mesh) then return true end
	end
	return true
end
function gui.WIModelEditor:RenderWireframe()
	local shaderWireframe = self.m_shaderWireframeAnimated
	if(shaderWireframe == nil or util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:RenderEntity(shaderWireframe,nil,nil,function(shaderWireframe,drawCmd)
    return shaderWireframe:RecordPushConstants(self.m_dsColorWireframe,shader.TexturedLit3D.PUSH_CONSTANTS_USER_DATA_OFFSET)
  end)
end
function gui.WIModelEditor:RenderVertices()
	local shaderVertices = self.m_shaderVerticesAnimated
	if(shaderVertices == nil or util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:RenderEntity(shaderVertices,nil,nil,function(shaderVertices,drawCmd)
    return shaderVertices:RecordPushConstants(self.m_dsColorVertices,shader.TexturedLit3D.PUSH_CONSTANTS_USER_DATA_OFFSET)
  end)
end
function gui.WIModelEditor:GetModelView() return self.m_pModelView end
function gui.WIModelEditor:OnSceneRender(drawCmd,cam,rt)
	for _,p in ipairs(self.m_tPanels) do
		if(util.is_valid(p) == true) then p:Render(drawCmd,cam,self.m_pModelView) end
	end

	local mdl = self:GetModel()
	if(util.is_valid(mdl) == false) then return end
	if(self.m_bShowWireframe == true) then self:RenderWireframe() end
	if(self.m_bShowVertices == true) then self:RenderVertices() end
...
function gui.WIModelEditor:Clear()
	self:SetModel()
end
function gui.WIModelEditor:SaveModel(mdlName)
	if(util.is_valid(self.m_model) == false) then return end
	if(file.get_file_extension(mdlName) ~= "wmd") then mdlName = mdlName .. ".wmd" end
	self.m_model:Save(mdlName)
end
function gui.WIModelEditor:UpdateModel()
	if(util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:UpdateModel()
end
function gui.WIModelEditor:SetModel(mdlName)
	local mdl
	if(mdlName ~= nil) then
		if(type(mdlName) == "string") then mdl = game.load_model(mdlName)
		else
			mdl = mdlName
			if(mdl ~= self.m_model) then mdl = mdl:Copy() end
		end
	else mdl = game.create_model(false) end
	if(self.m_model == mdl) then return end
	self.m_model = mdl
	self.m_modelName = mdlName
	if(util.is_valid(self.m_pModelView)) then self.m_pModelView:SetModel(mdl) end
	for _,p in ipairs(self.m_tPanels) do
		if(util.is_valid(p) == true) then p:SetModel(mdl) end
	end
	self:SetLOD(0)
end
function gui.WIModelEditor:PlayAnimation(anim)
	if(util.is_valid(self.m_pModelView) == false) then return end
	self.m_pModelView:PlayAnimation(anim)
end
function gui.WIModelEditor:OnSizeChanged(w,h)
	gui.WIBaseEditor.OnSizeChanged(self,w,h)
