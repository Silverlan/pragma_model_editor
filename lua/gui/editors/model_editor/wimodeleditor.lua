...
function gui.WIModelEditor:OnInitialize()
	...
	self.m_menuBar:AddItem(locale.get_text("view"), function(pContext)
		pContext:AddItem(locale.get_text("mde_reset_camera"), function(pItem)
			if util.is_valid(self) == false or util.is_valid(self.m_pModelView) == false then
				return
			end
	...
	self.m_container = gui.create(
		...
		self:GetWidth(),
		self:GetHeight() - self.m_menuBar:GetBottom(),
		0,
		0,
		1,
		1
	...
	if colComponent ~= nil then
		colComponent:SetColor(light.color_temperature_to_color(2700))
	end
	local radiusComponent = ent:GetComponent(ents.COMPONENT_RADIUS)
	if radiusComponent ~= nil then
		radiusComponent:SetRadius(300)
	end
...
function gui.WIModelEditor:GetModelName()
...
function gui.WIModelEditor:GetModelView()
