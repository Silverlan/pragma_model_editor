...
function gui.WIModelEditorBlendControllers:OnInitialize()
	...
	pSlider:AddCallback("OnChange",function(pSlider,progress,value)
		if(self:IsValid() == false or #self.m_activeBlendController == 0) then return end
		self:CallCallbacks("OnBlendControllerChanged",self.m_activeBlendController,value)
		if(util.is_valid(pBlendControllers) == false) then return end
		local pRow = pBlendControllers:GetFirstSelectedRow()
		if(util.is_valid(pRow) == false) then return end
...
function gui.WIModelEditorBlendControllers:ShowControls(b)
	if(util.is_valid(self.m_pLbActiveController) == true) then self.m_pLbActiveController:SetVisible(b) end
	if(util.is_valid(self.m_pLbCurrentValue) == true) then self.m_pLbCurrentValue:SetVisible(b) end
	if(util.is_valid(self.m_pSlider) == true) then self.m_pSlider:SetVisible(b) end
end
function gui.WIModelEditorBlendControllers:Reset()
	if(util.is_valid(self.m_pSlider) == true) then self.m_pSlider:SetValue(0) end
...
function gui.WIModelEditorBlendControllers:SetActiveBlendController(bc)
	if(bc == self.m_activeBlendController) then return end
	self.m_activeBlendController = bc
	if(util.is_valid(self.m_pSlider) == false) then return end
		...
		if(blendController ~= nil) then
			...
			if(util.is_valid(pRow) == true) then
				...
				if(util.is_valid(self.m_pLbActiveController) == true) then
					self.m_pLbActiveController:SetText(locale.get_text("active_blend_controller") .. ": " .. blendController.name)
...
function gui.WIModelEditorBlendControllers:OnSizeChanged(w,h)
	...
	if(util.is_valid(self.m_pButtonReset) == false) then return end
