...
function gui.WIModelEditorSkeleton:Render(drawCmd, cam, pModelView)
	...
	if
		self:GetShowItems() == false
		or util.is_valid(self.m_bufVerts) == false
		or util.is_valid(self.m_bufBoneIndices) == false
	then
		return
	end
	self.m_shader:Draw(
...
local function add_bones(self, pList, bone)
	...
	local function set_bone_color(boneId, color)
		if self:IsValid() == false or util.is_valid(ent) == false or util.is_valid(self.m_bufVerts) == false then
			return
		end
		local mdlComponent = ent:GetComponent(ents.COMPONENT_MODEL)
		local mdl = (mdlComponent ~= nil) and mdlComponent:GetModel() or nil
		if self.m_bufBoneOffsets[boneId] == nil or util.is_valid(mdl) == false then
			return
		end
...
function gui.WIModelEditorSkeleton:UpdateSkeleton()
	local ent = self:GetEntity()
	if util.is_valid(ent) == false or self.m_shader == nil then
		return
	end
...
function gui.WIModelEditorSkeleton:SetModel(mdl)
	...
	if util.is_valid(pSkeleton) == false then
		return
	end
	pSkeleton:Clear()
